...
import {
  ...
  type IndiceType,
  type IndiceSubtype,
  getIndicesByPlayerId,
  createPlayerIndice,
  deletePlayerIndice,
  indiceTypeLabels,
  indiceSubtypeLabels,
} from "@/lib/player-indices"

interface PlayerIndicesManagerProps {
  playerId: string
  playerName: string
  userId: string
  onClose: () => void
...
export function PlayerIndicesManager({ playerId, playerName, userId, onClose }: PlayerIndicesManagerProps) {
  ...
  const [indices, setIndices] = useState<PlayerIndice[]>([])
  ...
  const loadIndices = async () => {
    ...
    setLoading(true)
    const data = await getIndicesByPlayerId(playerId, selectedType, selectedSubtype)
    setIndices(data)
    setLoading(false)
  }
  ...
  const handleUpload = async () => {
    ...
    const result = await createPlayerIndice(playerId, selectedType, selectedSubtype, observations, selectedFile, userId)

    if (result.success) {
      toast({
        title: "Éxito",
        description: "El índice individual se guardó correctamente",
      })
      setObservations("")
      ...
      toast({
        title: "Error",
        description: result.error || "No se pudo guardar el índice individual",
        variant: "destructive",
      })
    }
  ...
  const handleDelete = async (indice: PlayerIndice) => {
    if (!confirm("¿Está seguro de eliminar este índice individual?")) return

    const result = await deletePlayerIndice(indice.id, indice.file_url)

    if (result.success) {
      toast({
        title: "Éxito",
        description: "El índice individual se eliminó correctamente",
      })
      loadIndices()
    } else {
      toast({
        title: "Error",
        description: result.error || "No se pudo eliminar el índice individual",
        variant: "destructive",
      })
    }
  }
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">
          ...
          <p>No hay registros individuales aún</p>
        </div>
      ) : (
              ...
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription className="text-xs">Registrado por el staff</CardDescription>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
        ...
        <Dialog open={showUploadForm} onOpenChange={setShowUploadForm}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              ...
              <DialogDescription>Registrar índice individual para {playerName}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div>
                ...
                <Textarea
                  id="observations"
                  placeholder="Ingrese observaciones sobre este índice individual..."
                  value={observations}
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-2xl">Índices Individuales</DialogTitle>
              <DialogDescription>Jugador: {playerName}</DialogDescription>
            </div>
            {selectedType && (
