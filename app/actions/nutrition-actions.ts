...
export async function getPlayerNutritionReportsAction(playerId: string): Promise<NutritionReport[]> {
...
export async function saveNutritionReportAction(report: Partial<NutritionReport>) {
  const supabase = await createServerClient()
  ...
  revalidatePath(`/player/${report.playerId}`)
